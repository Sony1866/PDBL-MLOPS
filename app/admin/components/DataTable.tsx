...
export default function DataTable({
  ...
  return (
    <div className="glass-card-static rounded-2xl overflow-hidden" style={{ boxShadow: '0 4px 20px rgba(14,165,233,0.06)' }}>
      {/* Header */}
      <div className="px-5 py-3.5 border-b border-sky-100/30 dark:border-sky-800/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        {title && <h4 className="text-xs font-extrabold text-sky-900 dark:text-sky-100">{title}</h4>}
        <div className="relative w-full sm:w-60">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-sky-400/50" />
          <input
            type="text" value={search}
            onChange={(e) => { setSearch(e.target.value); setPage(0); }}
            className="w-full pl-9 pr-3 py-2 rounded-lg bg-sky-50/60 dark:bg-sky-900/20 border border-sky-200/30 dark:border-sky-700/20 text-xs font-medium text-sky-900 dark:text-sky-100 placeholder:text-sky-400/40 transition-all"
            placeholder="Cari..."
          />
        </div>
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-sky-100/30 dark:border-sky-800/20">
              {columns.map(col => (
                <th key={col.key} className="px-4 py-3 text-left font-extrabold text-sky-600/70 dark:text-sky-400/60 uppercase tracking-wider text-[10px]">
                  {col.sortable ? (
            ...
            {pageData.length === 0 ? (
              ...
              pageData.map((row, ri) => (
                <tr key={ri} className="border-b border-sky-50/30 dark:border-sky-800/10 hover:bg-sky-50/30 dark:hover:bg-sky-900/10 transition-colors">
                  {columns.map(col => (
                    <td key={col.key} className="px-4 py-3 font-medium text-sky-800 dark:text-sky-200 whitespace-nowrap">
