      ...
      <div className="relative z-10 flex items-center justify-center min-h-screen px-4 py-24">
        <div className="w-full max-w-md">
          <div className="text-center mb-6">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-100/50 dark:bg-amber-900/20 border border-amber-200/40 dark:border-amber-700/20 mb-4">
              <Shield className="w-3 h-3 text-amber-500" /><span className="text-[10px] font-bold text-amber-500/80 uppercase tracking-[0.15em]">Admin Panel</span>
            </div>
          ...
          <div className="gradient-border" style={{ background: 'linear-gradient(135deg, #f59e0b, #ef4444, #8b5cf6, #f59e0b)', backgroundSize: '300% 300%', animation: 'gradientShift 4s ease infinite' }}>
            <div className="glass-card-static rounded-[22px] p-6">
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="email" value={form.email} onChange={(e) => setForm(f => ({ ...f, email: e.target.value }))}
                      className="w-full pl-10 pr-4 py-3 rounded-xl bg-sky-50/60 dark:bg-sky-900/20 border border-sky-200/40 dark:border-sky-700/30 text-sky-900 dark:text-sky-100 text-sm font-medium placeholder:text-sky-400/40 transition-all" placeholder="admin@creditsense.ai" />
                  </div>
                </div>
                <div>
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin-slow" /> : <><Shield className="w-4 h-4" /> Login Admin <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></>}
                </button>
              </form>
