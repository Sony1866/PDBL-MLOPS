...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'eda' | 'predictions' | 'users'>('overview');

  // ── Load users from localStorage ──
  const [registeredUsers, setRegisteredUsers] = useState<{id:string;email:string;fullName:string;createdAt:string;profile:{profileCompleted:boolean}}[]>([]);

  useEffect(() => {
    try {
      const users = JSON.parse(localStorage.getItem('mlops_users') || '[]');
  ...
  return (
    <div className="min-h-screen relative transition-colors duration-300 noise-overlay">
      ...
      <AdminSidebar mobileOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} onLogout={handleLogout} activeTab={activeTab} onTabChange={(t) => setActiveTab(t as typeof activeTab)} />

      <main className="lg:ml-[260px] relative z-10 min-h-screen">
        {/* Top bar */}
        <div className="sticky top-0 z-30 glass-card-static border-b border-slate-200/30 dark:border-slate-700/20 px-4 lg:px-8 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-xl hover:bg-slate-100/40 dark:hover:bg-slate-800/20 transition-colors">
              <Menu className="w-5 h-5 text-slate-600 dark:text-slate-300" />
            </button>
            <div>
              <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-indigo-100/50 dark:bg-indigo-900/20 border border-indigo-200/30 dark:border-indigo-700/20">
                <Shield className="w-2.5 h-2.5 text-indigo-500" />
                <span className="text-[9px] font-bold text-indigo-500/80 uppercase tracking-wider">Admin Dashboard</span>
              </div>
              <h1 className="text-lg font-black text-slate-800 dark:text-slate-100 mt-0.5" style={{ letterSpacing: '-0.03em' }}>
                Monitoring Panel
              </h1>
            </div>
          </div>
          <button onClick={loadData} className="flex items-center gap-2 px-4 py-2 rounded-xl glass-card-static text-xs font-bold text-slate-600 dark:text-slate-300 hover:shadow-md transition-all">
              ...
              <button key={tab.key} onClick={() => {
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${
                  activeTab === tab.key
                    ...
                    : 'glass-card-static text-slate-600 dark:text-slate-400 hover:shadow-md'
                }`}>
          ...
          {activeTab === 'users' && (
            <section id="users">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-black text-slate-800 dark:text-slate-100">Pengguna Terdaftar</h2>
                <span className="text-[10px] font-bold text-indigo-400/60 px-2 py-0.5 rounded-full bg-indigo-100/40 dark:bg-indigo-800/20">{registeredUsers.length}</span>
              </div>

              {registeredUsers.length === 0 ? (
                <div className="glass-card-static rounded-2xl p-12 text-center">
                  <Users className="w-12 h-12 text-slate-300/40 mx-auto mb-3" />
                  <p className="text-sm font-bold text-slate-600/50 dark:text-slate-400/40">Belum ada pengguna terdaftar</p>
                  <p className="text-xs text-slate-400/40 mt-1">Data akan muncul setelah ada user yang mendaftar</p>
                </div>
              ) : (
                <>
                  {/* User stats */}
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                    {[
                      { l: 'Total User', v: registeredUsers.length, g: 'from-indigo-400 to-violet-500', icon: Users },
                      { l: 'Profil Lengkap', v: registeredUsers.filter(u => u.profile?.profileCompleted).length, g: 'from-emerald-400 to-green-500', icon: UserCheck },
                      { l: 'Profil Belum Lengkap', v: registeredUsers.filter(u => !u.profile?.profileCompleted).length, g: 'from-amber-400 to-orange-500', icon: UserX },
                      { l: 'Pinjaman Aktif', v: (() => {
                        let count = 0;
                        registeredUsers.forEach(u => {
                          try {
                            const preds = JSON.parse(localStorage.getItem(`mlops_predictions_${u.id}`) || '[]');
                            const cleared = JSON.parse(localStorage.getItem(`mlops_loan_cleared_${u.id}`) || '[]');
                            const hasActive = preds.some((p: {result: string; id: string}) => p.result === 'LAYAK' && !cleared.includes(p.id));
                            if (hasActive) count++;
                          } catch { /* skip */ }
                        });
                        return count;
                      })(), g: 'from-sky-400 to-blue-500', icon: BadgeCheck },
                    ].map((c, i) => (
                      <div key={i} className="glass-card-static rounded-2xl p-4 text-center relative overflow-hidden">
                        <div className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${c.g}`} />
                        <div className="flex items-center justify-center gap-2">
                          <c.icon className="w-4 h-4 text-slate-400/60" />
                          <p className="text-xl font-black text-slate-800 dark:text-slate-100">{c.v}</p>
                        </div>
                        <p className="text-[9px] font-bold text-slate-500/60 uppercase tracking-wider mt-1">{c.l}</p>
                      </div>
                    ))}
                  </div>

                  {/* User table */}
                  <div className="glass-card-static rounded-2xl overflow-hidden" style={{ boxShadow: '0 4px 20px rgba(99,102,241,0.06)' }}>
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-b border-slate-200/30 dark:border-slate-700/20">
                            <th className="px-4 py-3 text-left font-extrabold text-slate-500/70 uppercase tracking-wider text-[10px]">Nama</th>
                            <th className="px-4 py-3 text-left font-extrabold text-slate-500/70 uppercase tracking-wider text-[10px]">Email</th>
                            <th className="px-4 py-3 text-left font-extrabold text-slate-500/70 uppercase tracking-wider text-[10px]">Terdaftar</th>
                            <th className="px-4 py-3 text-left font-extrabold text-slate-500/70 uppercase tracking-wider text-[10px]">Profil</th>
                            <th className="px-4 py-3 text-left font-extrabold text-slate-500/70 uppercase tracking-wider text-[10px]">Prediksi</th>
                            <th className="px-4 py-3 text-left font-extrabold text-slate-500/70 uppercase tracking-wider text-[10px]">Pinjaman</th>
                            <th className="px-4 py-3 text-left font-extrabold text-slate-500/70 uppercase tracking-wider text-[10px]">Aksi</th>
                          </tr>
                        </thead>
                        <tbody>
                          {registeredUsers.map((u) => {
                            ...
                            try {
                              ...
                              const activePred = preds.find((p: {result: string; id: string}) => p.result === 'LAYAK' && !cleared.includes(p.id));
                              if (activePred) { hasActive = true; activePredId = activePred.id; }
                            } catch { /* skip */ }

                            return (
                              <tr key={u.id} className="border-b border-slate-100/30 dark:border-slate-800/10 hover:bg-slate-50/30 dark:hover:bg-slate-900/10 transition-colors">
                                <td className="px-4 py-3 font-bold text-slate-800 dark:text-slate-200 whitespace-nowrap">{u.fullName}</td>
                                <td className="px-4 py-3 text-slate-600 dark:text-slate-400 whitespace-nowrap">{u.email}</td>
                                <td className="px-4 py-3 text-slate-500 dark:text-slate-400 whitespace-nowrap">
                                  {new Date(u.createdAt).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-extrabold ${u.profile?.profileCompleted ? 'bg-emerald-100/60 dark:bg-emerald-900/20 text-emerald-600' : 'bg-amber-100/60 dark:bg-amber-900/20 text-amber-600'}`}>
                                    {u.profile?.profileCompleted ? <><CheckCircle2 className="w-3 h-3" /> Lengkap</> : <><XCircle className="w-3 h-3" /> Belum</>}
                                  </span>
                                </td>
                                <td className="px-4 py-3 font-bold text-slate-700 dark:text-slate-300 whitespace-nowrap">{predCount}</td>
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-extrabold ${hasActive ? 'bg-sky-100/60 dark:bg-sky-900/20 text-sky-600' : 'bg-slate-100/60 dark:bg-slate-800/20 text-slate-400'}`}>
                                    {hasActive ? <><BadgeCheck className="w-3 h-3" /> Aktif</> : '— Tidak ada'}
                                  </span>
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap">
                                  {hasActive && (
                                    <button
                                      onClick={() => {
                                        if (!confirm(`Tandai pinjaman ${u.fullName} sebagai LUNAS?`)) return;
                                        try {
                                          const cleared = JSON.parse(localStorage.getItem(`mlops_loan_cleared_${u.id}`) || '[]');
                                          cleared.push(activePredId);
                                          localStorage.setItem(`mlops_loan_cleared_${u.id}`, JSON.stringify(cleared));
                                          setRegisteredUsers([...registeredUsers]); // force re-render
                                        } catch { /* skip */ }
                                      }}
                                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-bold bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 transition-colors border border-emerald-200/30 dark:border-emerald-700/20"
                                    >
